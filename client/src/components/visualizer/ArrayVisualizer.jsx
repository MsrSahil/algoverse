...
const DecisionPanel = ({ step, arrayData }) => {
    ...
    return (
      ...
      >
        ...
        <p className="text-sm text-emerald-500">
          Every element is in its final position.
        </p>
      ...
      >
        ...
        <p className="text-sm text-slate-400">
          {finalizedValue !== undefined
            ? `The largest unsorted value bubbled to the end.`
            : 'This element will not move again.'}
        </p>
        {!isLastPass && (
          <p className="mt-1 text-xs font-semibold text-cyan-400">
            Unsorted region shrinks → Pass {nextPass} begins.
          </p>
        )}
      </div>
    )
  ...
  if (type === STEP_TYPES.SORTED && earlyTermination) {
    return (
      <div
        ...
        aria-live="polite"
      >
        <p className="text-xs font-bold uppercase tracking-[0.2em] text-emerald-400">✓ Early Exit</p>
        <p className="text-base font-bold text-white">No swaps occurred this pass.</p>
        <p className="text-sm text-slate-400">The array is already fully sorted.</p>
      </div>
    )
  }
    ...
    return (
      ...
      >
        ...
        <p className="text-sm font-semibold text-emerald-300">
          {finalizedValue} is in its final position.
        </p>
      </div>
    )
  }
  ...
  if (type === STEP_TYPES.SWAP) {
    ...
    return (
      <div
        ...
        role="status"
        aria-live="polite"
        aria-label={`Swapping: ${mRight} moves right, ${mLeft} moves left`}
      >
        <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-rose-400/70">Swapping</p>
  ...
  if (type === STEP_TYPES.COMPARE && leftValue !== undefined && rightValue !== undefined) {
    ...
    return (
      ...
      >
        <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-slate-400">Comparing</p>
        ...
        {isSwap ? (
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-xs text-amber-300/80">
              {leftValue} is larger — it must move right.
            </p>
          </div>
        ) : isEqual ? (
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-xs text-slate-500">Equal values — already in order.</p>
          </div>
        ) : (
          <div className="flex flex-col items-center gap-1">
            ...
            <p className="text-xs text-slate-400">
              {leftValue} is smaller — already in correct order.
            </p>
          </div>
        )}
      </div>
  ...
  if (type === STEP_TYPES.START) {
    return (
      <div className="flex flex-col items-center gap-2 rounded-2xl border border-white/8 bg-slate-900/50 px-6 py-5 text-center">
        <p className="text-sm font-bold text-white">Bubble Sort</p>
        <p className="text-xs leading-relaxed text-slate-400">
          Compare neighbors. Swap if out of order.<br />
...
/* ─────────────────────────────────────────────────────────────────────────
   ...
   ─────────────────────────────────────────────────────────────────────────*/
const PassHeader = ({ pass, totalPasses, isPassComplete }) => {
...
const ArrayVisualizer = ({
  ...
  const isSwapStep = type === STEP_TYPES.SWAP
      ...
      >
        ...
        >
          {arrayData.map((value, slotIndex) => {
            ...
            // Determine which swapping element is moving right (the larger one)
            // The larger element lands at the higher index post-swap
            const isMovingRight = isSwappingSlot && swapIndices.length === 2 && slotIndex === swapIndices[1]
            ...
            return (
              <div key={`slot-${slotIndex}`} className="flex items-end">
                ...
                >
                  ...
                  {/* Swapping motion arrow — indicates direction for the moving-right element */}
                  {isMovingRight && (
                    <span
                  ...
                  {/* Swapping indicator dot for the moving-left element */}
                  {isSwappingSlot && !isMovingRight && (
                    <span
